...
function temperature_gaussian(x) 
	return 1 - math.exp(-16 * math.pow(x-0.5, 2))
end
...
function step()
	...
	proximity_right =  (robot.proximity[RIGHT].value + robot.proximity[20].value + robot.proximity[22].value + robot.proximity[19].value + robot.proximity[23].value)/5
	proximity_left =  (robot.proximity[LEFT].value + robot.proximity[5].value + robot.proximity[3].value + robot.proximity[2].value + robot.proximity[6].value)/5
	speed_proximity_left = proximity_left
	speed_proximity_right = proximity_right
	
	speed_light_left = 0.4 + (1 - robot.light[LEFT].value)
	speed_light_right = 0.4 + (1 - robot.light[RIGHT].value)
	
	speed_temperature_left = 0.3 + temperature_gaussian(temperature(position['left']))
	speed_temperature_right = 0.3 + temperature_gaussian(temperature(position['right']))
	
	speed_left = speed_proximity_left * 100 + speed_light_left * 5 + speed_temperature_left * 5
	speed_right = speed_proximity_right * 100 + speed_light_right * 5 + speed_temperature_right * 5
	robot.wheels.set_velocity(speed_left , speed_right )
end
